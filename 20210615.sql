...
/*
1. subquery는 ( )내에서 작성한다.
2. in,any,all : subquery의 결과가 여러개 일때
   =, >=, <= : subquery의 결과가 1개 이하일때
*/

# 데이터전처리대전 한빛미디어
# R 파이썬 데이터베이스 전처리하는거 정리를 해놨대
# 똑같은 요구가있으면 각각 프로그램에서 어떻게 다르게 사용할 수 있는지
# 그거한번 보면 도움될거라고 보라고하심 
# 데이터 전처리는 엔지니어든 분석가든 다 해야함 
...
# 파이썬에서넘겨준값을 DB에 저장을 시켜저야해
# python -> mysql
# mysql -> python
# 커넥터 사용해서 값을 주고 받아서 사용하고 할 수 있는거임
# 두개를 연결해줄 수 있는 커넥터라는게 있거든
...
#shop 나사 눌러보면 utf8mb4 이게 한글이 기본값이 된다는?거래 
...
# [ERR][SQL Editor Form]: Error Code: 1366.
# 인코딩이 어떻게되어있느냐에따라 문자를 읽느냐 마느냐 그래서
# 나사표시눌러서 한글삽입햇ㄴ느데 한글이 안되면 나사표시눌러서 charset을 latin1로 바꿔보자 해서 해본거래
# 원래는 utf8~ 그게 맞음 
# 근데 내컴에서는 이렇게 해도 잘되네 utf8로해도
